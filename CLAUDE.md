...
- **Every row says where it came from** (user request 2026-09-25): helpers "Part of Code · doospend-ios"; stray processes "Gradle daemon 9.3.1 · from Code · doospend" (role from argv for interpreters via `KERN_PROCARGS2`, app from the private `responsibility_get_pid_responsible_for_pid` — looked up with `dlsym`, it survives a daemon detaching to launchd — and never used to fold the row into that app, so Force Quit still ends only the daemon); Simulator processes "iOS Simulator · <device> · iOS 26.5" (device from the nearest `launchd_sim`'s args → `device.plist`). The folder is the process's own cwd, else its nearest parent's, else (detached, cwd `/`, e.g. ibtoold) its open files, shown as the enclosing git repo's name. A process whose path can't be read (root daemons like diagnosticd) is `.system`: no Force Quit.
...
- **Hard links count once** (like `du`): pnpm hard-links one copy of each package file into every place that uses it, and summing every path read an 11 GB node_modules as 23 GB. Files with `linkCount > 1` are `lstat`ed and their (dev, inode) claimed in a shared set; only the first path met counts.
- **Each Safe to Clean group has an ⓘ** (`CleanupGroup.guide`) opening a popover: what it is / what deleting costs / when to delete — **in Thai** (user request 2026-09-25); the rest of the UI stays English.
